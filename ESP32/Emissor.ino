...
uint8_t MAC_NODE1[] = {0x30, 0x76, 0xf5, 0xe8, 0x10, 0xe4}; // 30:76:f5:e8:10:e4 Nó Gateway (Nó 1)

// __attribute__((packed)) para evitar "padding" (lixo) na memória
...
// Variáveis para controlo de reconexão automática
volatile int falhasEnvioESPNow = 0;
volatile bool precisaReconectar = false;
...
void setupTimersHW() {
  ...
  timerAlarm(timerSend, 1000000, true, 0); // 4 segundos para envio de dados
}
...
// Função para recuperar o link se o router mudar de canal
void reconectarCanalDinamico() {
  Serial.println("\n[AVISO] Muitas falhas seguidas no ESP-NOW! O router pode ter mudado de canal.");
  Serial.println("A procurar novo canal...");
  
  int32_t novoCanal = obterCanalDoRouter(WIFI_SSID_ALVO);
  esp_wifi_set_channel(novoCanal, WIFI_SECOND_CHAN_NONE);
  
  peerNode1.channel = novoCanal;
  if (esp_now_mod_peer(&peerNode1) == ESP_OK) {
    Serial.printf(">>> Sucesso! Peer atualizado para o novo canal: %d\n", novoCanal);
  } else {
    Serial.println(">>> Erro ao atualizar o Peer!");
  }
...
void setupEspNow() {
  ...
  // Assinatura atualizada e lógica de contagem de falhas
  esp_now_register_send_cb([](const wifi_tx_info_t *tx_info, esp_now_send_status_t status) {
    if (status == ESP_NOW_SEND_SUCCESS) {
      falhasEnvioESPNow = 0; // Reset se sucesso
    } else {
      falhasEnvioESPNow++; // Incrementa se falhou
      if (falhasEnvioESPNow >= MAX_FALHAS_PERMITIDAS) {
        precisaReconectar = true; // Aciona flag para rescan no loop()
      }
    }
  });
  ...
  Serial.printf(">>> Canal ESP-NOW configurado dinamicamente para o Gateway: %d\n", canalDinamico);
}
...
void loop(void) {
  ...
  // Resgata o link se houver mudança de canal
  if (precisaReconectar) {
    reconectarCanalDinamico();
  }
