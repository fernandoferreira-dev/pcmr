...
const METRICAS: { key: MetricaKey; label: string; cor: string; unidade: string }[] = [
  { key: 'temperatura', label: 'Temperatura', cor: '#f97316', unidade: '°C' },
  { key: 'bpm', label: 'Frequência Cardíaca', cor: '#dc2626', unidade: 'bpm' },
  { key: 'magnitudeG', label: 'Magnitude (aceleração)', cor: '#2563eb', unidade: 'G' },
]
    ...
    const buscarLeitura = async () => {
      try {
        ...
        if (data.atualizadoEm !== ultimaHoraRef.current) {
          ...
          pulsoTimeoutRef.current = setTimeout(() => setPulsando(false), DURACAO_PULSO_MS)
        }
      } catch {
          ...
          {historico.length > 1 ? (
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={historico} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                ...
                <YAxis tick={{ fontSize: 11 }} width={40} unit={` ${metrica.unidade}`} />
                <Tooltip formatter={(value) => [`${value ?? '—'} ${metrica.unidade}`, metrica.label]}/>
                <Line
                  type="monotone"
