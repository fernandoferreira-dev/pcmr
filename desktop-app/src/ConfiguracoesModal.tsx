...
export default function ConfiguracoesModal({
  ...
  const carregarConfig = async () => {
    try {
      ...
      setConfig(data)
      setTemperaturaInput(data.temperaturaMaxAlerta.toString())
      setBpmInput(data.bpmMaxAlerta.toString())
    } catch {
  ...
  const guardarConfig = async () => {
    ...
    if (isNaN(temperatura) || isNaN(bpm)) {
      setErroConfig('Introduz valores numéricos válidos.')
      return
    }
    ...
    try {
      const res = await fetch('/api/configuracoes', {
        ...
        body: JSON.stringify({
          temperaturaMaxAlerta: temperatura,
          bpmMaxAlerta: bpm,
      ...
      if (!res.ok) {
        setErroConfig('Não foi possível guardar a configuração.')
        return
      }
          ...
          {aba === 'limites' && (
            <div className="flex flex-col gap-4">
              <p className="text-sm text-gray-500">
                Define os valores máximos que, quando excedidos, geram um alerta clínico
                registado automaticamente na base de dados.
              </p>
              ...
              <label className="flex flex-col gap-1">
                <span className="text-sm font-semibold text-gray-700">Frequência cardíaca máxima (bpm)</span>
                <input
                  type="number"
                  value={bpmInput}
