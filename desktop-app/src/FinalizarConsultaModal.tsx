...
export default function FinalizarConsultaModal({
  ...
  const procurarPacientes = useCallback(async (nome: string) => {
    try {
      const res = await fetch(`/api/pacientes/procurar?nome=${encodeURIComponent(nome)}`)
      if (!res.ok) return
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/40 flex items-center justify-center p-6">
      <div className="bg-white rounded-3xl shadow-xl w-full max-w-lg p-6 relative">
        ...
        <h2 className="text-xl font-bold text-gray-800 mb-4">Associar Paciente</h2>

        <div className="flex gap-2 mb-4">
          <button
            onClick={() => setModo('procurar')}
            className={`flex-1 py-2 rounded-full text-sm font-medium transition-colors ${
              modo === 'procurar' ? 'bg-[#AAB99F] text-white' : 'bg-gray-100 text-gray-600'
            }`}
          >
            Paciente existente
          </button>
          <button
            ...
            className={`flex-1 py-2 rounded-full text-sm font-medium transition-colors ${
              modo === 'novo' ? 'bg-[#AAB99F] text-white' : 'bg-gray-100 text-gray-600'
            }`}
          >
            Novo paciente
          </button>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="max-h-48 overflow-y-auto flex flex-col gap-1">
              ...
              {resultados.length === 0 && termo.trim() !== '' && (
                <p className="text-xs text-gray-400 px-1">Nenhum paciente encontrado.</p>
              )}
            </div>
          </div>
        ) : (
